...
checkpoint = './BEST_checkpoint_IU-XRAY.pth.tar'  # model checkpoint
word_map_file = './data/wordmap.json'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
# trans_resize = 256
# resize = transforms.Resize((trans_resize, trans_resize))
def evaluate(beam_size):
    ...
    loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, 'TEST', transform=transform_sequence),
    ...
    for i, (image, caps, caplens, allcaps) in enumerate(
        ...
        while True:
            ...
            # print(seqs[prev_word_inds.type(torch.long)])
            # quit()
        ...
        img_captions = list(
            map(lambda c: [w for w in c if w not in {word_map['<sta>'], word_map['<end>'], word_map['<pad>']}],
                img_caps))  # remove <sta> and pads
        references.append(img_captions)

        # Hypotheses
        hypotheses.append([w for w in seq if w not in {word_map['<sta>'], word_map['<end>'], word_map['<pad>']}])
...
if __name__ == '__main__':
    ...
    print("\n@ beam size of %d BLEU-1 score is %.4f, BLEU-2 score is %.4f, BLEU-3 score is %.4f, BLEU-4 score is %.4f." % (beam_size, bleu1, bleu2, bleu3, bleu4))
