...
# Training parameters
start_epoch = 0
epochs = 250  # number of epochs to train for (if early stopping is not triggered)
...
def main():
    ...
    if checkpoint is None:
        ...
        encoder_optimizer = torch.optim.Adam( encoder.network_model.parameters(),
                                             lr=encoder_lr) 
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, 'TRAIN', transform=transform_sequence),
        batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
    val_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, 'VAL', transform=transform_sequence),
        batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
    # Epochs
    for epoch in range(start_epoch, epochs):
        ...
        # save_checkpoint('IU-XRAY', epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer, recent_bleu4, is_best)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # batch_size, num_channels, height, width = imgs.size()    
        # imgs = torch.autograd.Variable(imgs.view(-1, num_channels, height, width).cuda())
        ...
        scores, caps_sorted, decode_lengths, alphas, sort_ind = decoder(embeddings, caps, caplens)
        print('scores ', scores.shape)
        print('caps_sorted ', caps_sorted.shape)
        print('decode_lengths ', decode_lengths.shape)
        print('alphas :', alphas.shape)
        print('sort ind ', sort_ind.shape)
        paes
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            for j in range(allcaps.shape[0]):
                ...
                img_captions = list(
                    map(lambda c: [w for w in c if w not in {word_map['<sta>'], word_map['<pad>']}],
